...
>
    ...
    >
        ...
        <form wire:submit="save">
            @if (session()->has('message'))
                <div x-data="{ show: true }" x-show="show" x-transition.duration.500ms x-init="setTimeout(() => show = false, 5000)" class="mb-6 p-4 bg-green-50 text-green-700 rounded-xl border border-green-200 flex items-center shadow-sm">
            ...
            {{-- 日記本文 --}}
            <div class="mb-6">
                <label for="text" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1 ml-1">Diary Content</label>
                <div class="relative">
                    <textarea wire:model="text" id="text" rows="4" class="w-full p-4 border-gray-200 rounded-xl shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 resize-none bg-gray-50 focus:bg-white transition-colors" placeholder="今日の出来事や、感じたことを自由に書いてみましょう..."></textarea>
    ...
    {{-- 2. 投稿不可メッセージ --}}
    <div x-show="!isVisible" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100" style="display: none;" class="bg-gray-50 p-8 rounded-2xl border-2 border-dashed border-gray-200 text-center">
        ...
        <p class="text-sm text-gray-500 mb-6 leading-relaxed">
            日記の投稿は「今日」の日付でのみ可能です。<br>過去の記録は閲覧のみとなります。
        </p>
        <button @click="$dispatch('date-selected', { date: null });" class="inline-flex items-center px-6 py-3 bg-white border border-gray-300 rounded-xl font-bold text-gray-700 shadow-sm hover:bg-gray-50 hover:text-indigo-600 hover:border-indigo-300 transition-all duration-200 group">
